...
const signupSchema = credentialsSchema.extend({
  displayName: z.string().trim().min(2, { message: 'Nombre muy corto' }).max(80),
});
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (msg.toLowerCase().includes('invalid login')) {
        ...
        toast.error('Error guardando perfil. Intenta de nuevo o contacta soporte.');
      } else {
        toast.error(msg);
      }
    } finally {
      ...
      >
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-muted-foreground text-sm">
            Tus canciones, listas y favoritos sincronizados.
          </p>
        </div>

        <div className="w-full flex p-1 bg-secondary rounded-xl">
          <button
        ...
        <form onSubmit={handleEmailSubmit} className="w-full space-y-3">
          {mode === 'signup' && (
            <input
              ...
              maxLength={80}
              required
              className="w-full px-4 py-3 rounded-xl bg-secondary border border-border text-foreground text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
            />
          )}
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full py-3 rounded-xl gold-gradient text-primary-foreground font-semibold text-sm hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
          >
        ...
        <button
          ...
          type="button"
          className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl bg-secondary hover:bg-surface-hover text-foreground font-medium transition-all active:scale-[0.98]"
        >
        ...
        <button
          onClick={handleGuest}
          type="button"
          className="w-full px-6 py-2.5 rounded-xl border border-border text-muted-foreground hover:text-foreground hover:border-muted-foreground text-sm font-medium transition-all"
        >
